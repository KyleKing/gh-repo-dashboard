...
# the public half as a write-enabled deploy key on the tap repo (creating the
# repo if needed), archives the private half in 1Password, and stores it as an
# Actions secret on the current repo. Re-running rotates the key: the old
# deploy key with the same title is replaced and the secret is overwritten.
#
# Usage: provision-tap-deploy-key.sh [tap-repo]
#   tap-repo     defaults to <owner>/homebrew-tap for the current repo's owner
# Environment:
#   SECRET_NAME  Actions secret to set (default: TAP_DEPLOY_KEY)
#   OP_VAULT     1Password vault for the archived key (default: Private)

set -euo pipefail

SECRET_NAME="${SECRET_NAME:-TAP_DEPLOY_KEY}"
OP_VAULT="${OP_VAULT:-Private}"

for tool in gh op ssh-keygen; do
...
if ! gh repo view "$tap_repo" >/dev/null 2>&1; then
    ...
    gh repo create "$tap_repo" --public --description "Homebrew tap for $owner's tools"
fi
